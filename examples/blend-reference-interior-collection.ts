...
for (const definition of BLEND_REFERENCE_INTERIORS) {
  const parts = buildBlendReferenceInteriorParts({ kind: definition.defaults.kind });
...
const ids = new Set(BLEND_REFERENCE_INTERIORS.map((entry) => entry.id));
manifest.models = manifest.models.filter((model) => !ids.has(model.id));
manifest.models.push(...BLEND_REFERENCE_INTERIORS.map((entry) => ({
  id: entry.id,
  name: entry.name,
  file: `${entry.id}.json`,
})));
