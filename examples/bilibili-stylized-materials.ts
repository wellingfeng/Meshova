import { mkdirSync, writeFileSync } from "node:fs";
import path from "node:path";
import {
  BILIBILI_MATERIAL_DEFINITIONS,
  BILIBILI_MATERIALS,
  exportPBR,
  materialFromFields,
  validateMaterial,
  type BilibiliMaterialName,
} from "../src/index.js";

const size = Number(process.argv[2] ?? 256);
const requestedName = process.argv[3] as BilibiliMaterialName | undefined;
const names = requestedName
  ? [requestedName]
  : Object.keys(BILIBILI_MATERIALS) as BilibiliMaterialName[];
const outputRoot = path.resolve(process.cwd(), "out", "materials", "bilibili-stylized-38");

if (!Number.isInteger(size) || size < 16 || size > 2048) {
  throw new Error("尺寸必须是 16–2048 的整数。 ");
}
if (requestedName && !BILIBILI_MATERIALS[requestedName]) {
  throw new Error(`未知材质：${requestedName}`);
}

for (const name of names) {
  const material = materialFromFields(size, BILIBILI_MATERIALS[name]({}));
  const problems = validateMaterial(material);
  if (problems.length > 0) throw new Error(`${name}: ${problems.join("; ")}`);
  const directory = path.join(outputRoot, name);
  mkdirSync(directory, { recursive: true });
  for (const [filename, bytes] of Object.entries(exportPBR(material, name).files)) {
    writeFileSync(path.join(directory, filename), bytes);
  }
  const definition = BILIBILI_MATERIAL_DEFINITIONS[name];
  console.log(`${String(definition.episode).padStart(2, "0")} ${definition.label} -> ${directory}`);
}
